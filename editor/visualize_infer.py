...
def seq2combined(gt, preds, mp4, bones, texts, fps=25):
    ...
    frames_dir = Path(mp4).with_suffix('')
...
if __name__ == "__main__":
    ...
    ap.add_argument("--all_mask", default=True, help="all mask")
    args = ap.parse_args()
    ...
    # ------ Save GT & predictions as .pk files -------
    gt_pk_path = vis_dir / f"{res_name}_gt.pk"
    with open(gt_pk_path, "wb") as f:
        pickle.dump(pose_all, f)
    print(f"✅ Saved GT: {gt_pk_path}")

    for text, pred in preds.items():
        pred_pk_path = vis_dir / f"{res_name}_{safe_name(text)}.pk"
        with open(pred_pk_path, "wb") as f:
            pickle.dump(pred, f)
        print(f"✅ Saved pred [{text}]: {pred_pk_path}")