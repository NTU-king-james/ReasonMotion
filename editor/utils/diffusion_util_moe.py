...
import math
# from moe_util import MoETransformerEncoderLayer
from moe_transformer import MoETransformerEncoderLayer
...
def get_torch_trans_moe(heads=8, layers=1, channels=64, num_experts=4, top_k=2):
    print("MoE torch transformer")
    encoder_layer = MoETransformerEncoderLayer(
        d_model=channels, nhead=heads, dim_feedforward=64, activation="gelu",
        num_experts=num_experts, top_k=top_k, batch_first=True
    )
    return nn.TransformerEncoder(encoder_layer, num_layers=layers)
...
class diff_CSDI(nn.Module):
    ...
    def forward(self, x, cond_info, diffusion_step, text_emb=None):
        # 1, 2, 72, 70
        # Channel 0 -> Have observed data (GT)
		# Channel 1 -> Region to predict (with noise)
    ...
    def get_load_balancing_loss(self):
        ...
        total_loss = 0.0
        count = 0
        total_fi = torch.zeros(4, dtype=torch.float32, device="cuda:0")  # Assume 4 experts, initialize to 0
        for layer in self.residual_layers:
...
class ResidualBlock(nn.Module):
    def __init__(self, side_dim, channels, diffusion_embedding_dim, nheads, textemb=384, moe=False):
        ...
        if moe == False:
            ...
            self.time_layer = get_torch_trans_moe(heads=nheads, layers=1, channels=channels, num_experts=4, top_k=2)
            self.feature_layer = get_torch_trans_moe(heads=nheads, layers=1, channels=channels, num_experts=4, top_k=2)
